...
class FetcherService {
    static fetchData = async (listName: string) => {
        const response = await axios.get<IItems>(`http://localhost:3001/${listName}`);
        return response.data;
    }

    static addItem = async (listName: string, item: object) => {
        const response = await axios.post<ISingleItem>(`http://localhost:3001/${listName}`, item);
        return response.data;
    }
    ...
    static editItem = async (listName: string, id: number, item: ISingleItem) => {
        ...
        const response = await axios.put<ISingleItem>(`http://localhost:3001/${listName}/${id}`, {
            ...item
        });
        return response.data;
    }
